...
class OmegaUrlFileProcessor constructor(private val client: HttpClient) : OmegaFileProcessor<OmegaUrlFile> {
    ...
    override suspend fun isExist(entity: OmegaUrlFile, extractor: OmegaResourceExtractor): Boolean? = client.request(Url(entity.url)) {
        method = HttpMethod.Head
    }.status.isSuccess()

    override suspend fun getInput(entity: OmegaUrlFile, extractor: OmegaResourceExtractor): Input? = client.request(Url(entity.url)) {
        /* nothing */
    }.readBytes().asInput()
